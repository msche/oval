...
public class JPAAnnotationsConfigurer implements Configurer
{
	...
	public ClassConfiguration getClassConfiguration(final Class< ? > clazz)
	{
		...
		for (final Field field : config.type.getDeclaredFields())
		{
			...
			if (checks.size() > 0)
			{
				if (config.fieldConfigurations == null) config.fieldConfigurations = new LinkedHashSet<>(8);

				final FieldConfiguration fc = new FieldConfiguration();
				fc.name = field.getName();
				fc.checks = checks;
				checks = new ArrayList<>(); // create a new list for the next field with checks
		...
		for (final Method method : config.type.getDeclaredMethods())
		{
			...
			if (checks.size() > 0)
			{
				if (config.methodConfigurations == null) config.methodConfigurations = new LinkedHashSet<>(2);
				...
				checks = new ArrayList<>(); // create a new list for the next method having return value checks
